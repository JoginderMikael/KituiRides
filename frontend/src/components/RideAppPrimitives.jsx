...
export function AppHeader({
  ...
  return (
    <div className={`rounded-[22px] border border-slate-200 bg-white p-5 shadow-[0_18px_34px_-28px_rgba(15,23,42,0.28)] ${className}`}>
...
export function SectionCard({ children, className = "" }) {
  return (
    <Card className={`rounded-[22px] border border-slate-200 bg-white p-5 shadow-[0_18px_34px_-28px_rgba(15,23,42,0.28)] ${className}`}>
      {children}
    </Card>
  );
}
...
export function CompactStatCard({ label, value, helper, tone = "muted" }) {
  return (
    <div className="rounded-[18px] border border-slate-200 bg-white p-4 shadow-[0_14px_28px_-26px_rgba(15,23,42,0.26)]">
...
export function PrimaryActionButton({ children, className = "", ...props }) {
  return (
    <Button
      size="lg"
      className={`min-h-[3.55rem] rounded-2xl bg-[#169b45] shadow-[0_20px_36px_-24px_rgba(22,155,69,0.8)] hover:bg-[#11803a] ${className}`}
      {...props}
    >
      {children}
    </Button>
  ...
  return (
    <button
      ...
      className={`w-full rounded-[24px] border p-4 text-left transition ${
        selected
          ? "border-emerald-500 bg-emerald-50 shadow-[0_18px_36px_-26px_rgba(22,155,69,0.32)]"
          : "border-slate-200 bg-white hover:border-slate-300"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <div className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${selected ? "bg-white text-emerald-700" : "bg-slate-100 text-slate-700"}`}>
            {icon}
          </div>
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <p className="font-semibold text-slate-950">{title}</p>
              {badge ? <Badge label={badge} size="sm" variant={selected ? "success" : "default"} /> : null}
            </div>
      ...
      {helper ? <p className={`mt-3 text-sm ${selected ? "text-emerald-800" : "text-slate-500"}`}>{helper}</p> : null}
    </button>
  );
}
...
export function BottomSheet({
  ...
  return (
    <div className={`rounded-[24px] border border-slate-200 bg-white shadow-[0_18px_34px_-28px_rgba(15,23,42,0.28)] ${className}`}>
      <button
        type="button"
        onClick={onToggle}
...
export function MobileBottomNav({ items, value, onChange }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-[68] border-t border-slate-200 bg-white px-2 pb-[calc(env(safe-area-inset-bottom,0px)+0.55rem)] pt-2 shadow-[0_-12px_24px_-22px_rgba(15,23,42,0.2)] md:hidden">
